...
public:
    ...
    as_(nh_, name, boost::bind(&FaceRecognition::executeCB, this, _1), false)
  {
    ...
    pnh_.param<double>("confidence_value", confidence_value, 0.8);
	 ...
	 ROS_WARN("Alert: Database is not updated. Please delete \"facedata.xml\" and re-run!"); //AFTER
       ...
       else
       {
         if (show_screen_flag) {
           ...
           text_image.str("");
           text_image <<"wellcome!  " <<frl.personNames[nearest-1].c_str();
           cvPutText(img, text_image.str().c_str(), cvPoint( 10, 50), &font, textColor);
         }
...
int main(int argc, char** argv)
{
  ...
  ros::NodeHandle n;
  ros::Publisher chatter_pub = n.advertise<std_msgs::String>("face_out", 1000);

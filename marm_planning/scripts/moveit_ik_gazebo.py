...
def do_it():
        ...
        target_pose.pose.position.z = obj_pose.position.z+0.02
        ...
        target_pose.header.frame_id = reference_frame
        target_pose.header.stamp = rospy.Time.now()     
        target_pose.pose.position.x = obj_pose.position.z+0.0
        target_pose.pose.position.y = obj_pose.position.x
        target_pose.pose.position.z = obj_pose.position.y+0.02
        print u"第2步规划。。。。。"
        # 设置机器臂当前的状态作为运动初始状态
        arm.set_start_state_to_current_state()
        # 设置机械臂终端运动的目标位姿
